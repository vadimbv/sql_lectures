...
## Задача 7: HARD - Комплексный запрос (JOINs + оконные функции + CASE)

### Описание
Создать расширенный квартальный отчет по продажам с трендами и скользящими показателями. Для каждого квартала показать:
- Квартал и год
- Название категории
- Количество заказов
- Сумму продаж
- Скользящую сумму продаж за текущий и предыдущий квартал (2-quarter rolling sum)
- Средние продажи за последние 3 квартала (3-quarter moving average)
- Процент от всех продаж в году
- Ранжирование категорий по продажам в квартале
- Тренд продаж: сравнение с предыдущим кварталом (рост/падение в %)
- Кумулятивная сумма продаж с начала года по текущий квартал
- Статус категории: 'Hot' (> средней по скользящему окну), 'Normal' (= средней), 'Cold' (< средней)
...
orders AS (
    -- Q1 2024
    SELECT 1 as order_id, 1 as category_id, '2024-01-15'::DATE as order_date, 1000 as amount
    UNION ALL
    SELECT 2, 2, '2024-02-10', 400
    UNION ALL
    SELECT 3, 3, '2024-03-05', 100
    -- Q2 2024
    UNION ALL
    SELECT 4, 1, '2024-04-10', 1100
    UNION ALL
    SELECT 5, 2, '2024-05-15', 450
    UNION ALL
    SELECT 6, 3, '2024-06-20', 120
    -- Q3 2024
    UNION ALL
    SELECT 7, 1, '2024-07-05', 1050
    UNION ALL
    SELECT 8, 2, '2024-08-12', 480
    UNION ALL
    SELECT 9, 3, '2024-09-18', 110
    -- Q4 2024
    UNION ALL
    SELECT 10, 1, '2024-10-08', 1150
    UNION ALL
    SELECT 11, 2, '2024-11-14', 500
    UNION ALL
    SELECT 12, 3, '2024-12-20', 130
    -- Q1 2025
    UNION ALL
    SELECT 13, 1, '2025-01-15', 1200
    UNION ALL
    SELECT 14, 1, '2025-02-10', 800
    UNION ALL
    SELECT 15, 2, '2025-01-20', 500
    UNION ALL
    SELECT 16, 2, '2025-03-05', 600
    UNION ALL
    SELECT 17, 3, '2025-01-25', 150
    -- Q2 2025
    UNION ALL
    SELECT 18, 1, '2025-04-10', 950
    UNION ALL
    SELECT 19, 3, '2025-04-15', 200
    UNION ALL
    SELECT 20, 2, '2025-04-20', 700
    UNION ALL
    SELECT 21, 1, '2025-05-05', 1100
    UNION ALL
    SELECT 22, 3, '2025-05-10', 300
    -- Q3 2025
    UNION ALL
    SELECT 23, 1, '2025-07-08', 1300
    UNION ALL
    SELECT 24, 2, '2025-08-12', 750
    UNION ALL
    SELECT 25, 3, '2025-09-15', 350
)
```

## Требуемый результат
```
quarter | year | category_name   | orders | total_sales | rolling_2q_sum | moving_avg_3q | year_sales_pct | rank_in_q | trend_vs_prev_q | cumulative_ytd | status
--------|------|-----------------|--------|-------------|----------------|---------------|----------------|-----------|-----------------|----------------|--------
Q1      | 2024 | Electronics     | 1      | 1000        | 1000           | 1000.00       | 66.67          | 1         | N/A             | 1000           | Hot
Q1      | 2024 | Furniture       | 1      | 400         | 400            | 400.00        | 26.67          | 2         | N/A             | 400            | Normal
Q1      | 2024 | Office Supplies | 1      | 100         | 100            | 100.00        | 6.67           | 3         | N/A             | 100            | Cold
Q2      | 2024 | Electronics     | 1      | 1100        | 2100           | 1050.00       | 65.87          | 1         | +10.00%         | 2100           | Hot
Q2      | 2024 | Furniture       | 1      | 450         | 850            | 425.00        | 26.95          | 2         | +12.50%         | 850            | Normal
Q2      | 2024 | Office Supplies | 1      | 120         | 220            | 110.00        | 7.19           | 3         | +20.00%         | 220            | Cold
